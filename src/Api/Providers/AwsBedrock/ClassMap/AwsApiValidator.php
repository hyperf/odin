 ...
 *
 * Custom version with fixed checkDocumentType method for AWS Bedrock Converse API
 * This class replaces the original AWS SDK Validator to support empty objects in toolUse.input
 */
class Validator
{
    private $path = [];
    ...
    /**
     * Fixed version of checkDocumentType to support objects (especially empty objects)
     * This is essential for AWS Bedrock Converse API toolUse.input parameter.
     * @param mixed $value
     */
    private function checkDocumentType($value)
    {
        ...
        // 🔧 FIXED: 支持对象类型（特别是为了 AWS Bedrock Converse API 的 toolUse.input）
        if (is_object($value)) {
            // 允许 stdClass 对象（包括空对象）
